...
RSpec.describe ConnpassApiV2::Client::UserMethods do
  ...
  describe "#get_user_groups" do
    ...
    describe "#get_user_presenter_events" do
      let(:nickname) { "sue445" }

      context "without params" do
        subject { client.get_user_presenter_events(nickname) }

        before do
          stub_request(:get, "https://connpass.com/api/v2/users/sue445/presenter_events/").
            with(headers: request_headers).
            to_return(status: 200, headers: response_headers, body: fixture("get_user_presenter_events.json"))
        end
      ...
      context "with params" do
        subject do
          client.get_user_presenter_events(nickname, start: start, count: count)
        end
        ...
        before do
          stub_request(:get, "https://connpass.com/api/v2/users/sue445/presenter_events/?count=100&start=1").
            with(headers: request_headers).
            to_return(status: 200, headers: response_headers, body: fixture("get_user_presenter_events.json"))
        end
